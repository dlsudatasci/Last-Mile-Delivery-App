...
import { PolygonWhileAnnotating } from '@/components/map/polygon';
...
import { Animated, Keyboard, StyleSheet, TouchableOpacity, useColorScheme, View } from 'react-native';
...
export default function SelectPoint() {
    ...
    const { addAnnotation, selectRide } = useRidesStore();
    ...
    const handleDragEnd = (e: FeaturePayload) => {
        ...
        setSelectedPoint(closestPointCoordinates);
    };
    ...
    const handleSaveAnnotation = async (data: z.infer<typeof AddAnnotationSchema>) => {
        if (isSaving || !selectedPoint) return;
        setIsSaving(true);
        try {
            // Find the point in ride.points that matches the selected coordinates
            const point = ride.points.find(
                p => p.coordinate.longitude === selectedPoint[0] && p.coordinate.latitude === selectedPoint[1]
            );

            if (!point) {
                throw new Error('Selected point not found in ride points');
            }
            ...
            const annotation = await saveAnnotation({
                ...
                points: [point], // Pass single point in array
                ...
                <Animated.ScrollView
                    style={[
                        styles.modalContent,
                        {
                            ...
                            display: selectedPoint ? 'flex' : 'none',
                        },
                    ]}
                >
