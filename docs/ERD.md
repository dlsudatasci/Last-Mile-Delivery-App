...
erDiagram
    ...
    rides {
        ...
        boolean isPublic "Community feed visibility"
        boolean isGPXUpload "True if imported from GPX"
        boolean fromWeb "True if created via admin web"
        string rideVideo "Optional Storage URL"
        array autoGeneratedAnnotations "Optional AI segments"
    ...
    annotations {
        ...
        string userId FK "Points to users document ID"
        string annotationId "Local layout marker ID"
        string type "point or segment"
        array points "GPS coordinates"
        string mediaUri "Optional Storage URL"
        string mediaType "Optional MIME type"
        string sentiment "Optional AI sentiment output"
        array additionalTags "Optional checklist tags"
        array autoGeneratedAnnotations "Optional AI data"
        boolean autoAnnotationSeen "Optional"
        string source "Optional e.g. auto-generated-from-video"
        number timestamp "Optional Unix ms for point"
        number start_timestamp "Optional Unix ms for segment"
        number end_timestamp "Optional Unix ms for segment"
        number createdAt "Unix ms"
    }
    ...
    studyParticipants {
        ...
        string riderName "Full name for study record"
        string phoneNumber "Philippine mobile 09xxxxxxxxx"
    ...
    adminNotifications {
        string notificationId PK "UUID or quota-userId"
        string type "quota_reached or compensation_claim"
        string userId FK "Points to users document ID"
        string email "Nullable rider email"
        string status "unread or read"
        number recordedSubmissions "Optional for quota alerts"
        number requiredSubmissions "Optional threshold value 10"
        string referenceNumber "Optional claim ref"
        number amount "Optional claim PHP"
        timestamp createdAt "Firestore serverTimestamp"
    }

    users ||--o{ rides : "records"
    users ||--o| studyParticipants : "enrolls as"
    users ||--o| compensationClaims : "submits"
    users ||--o{ tickets : "creates"
    users ||--o{ adminNotifications : "triggers"
    rides ||--|| map_points : "GPS stored in subcollection"
    rides ||--o{ annotations : "tagged deviations in subcollection"
```
...
│           └── {annotationId}         ← Auto UUID: deviation markers
│
├── studyParticipants/
│   └── {userId}                       ← Firebase Auth UID as doc ID
│
...
│
├── adminNotifications/
│   └── {notificationId}               ← UUID or "quota-{userId}"
│
└── app_version/
...
| `rides/{rideId}/annotations` | Owner only | Full access (Admin SDK) |
...
| `adminNotifications/{id}` | Write only, no read | Full read/write; mark read |
...
| `users` | `studyParticipants` | One-to-One (optional) | A rider may enroll in the study once |
| `users` | `compensationClaims` | One-to-One (optional) | A rider submits at most one claim |
| `users` | `tickets` | One-to-Many | A rider creates multiple support tickets |
| `users` | `adminNotifications` | One-to-Many | System auto-creates notifications per event |
| `rides` | `map/points` | One-to-One | Each ride has exactly one GPS breadcrumb document |
| `rides` | `annotations` | One-to-Many | A ride can have many deviation annotations |
